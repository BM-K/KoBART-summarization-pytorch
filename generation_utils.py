    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while cur_len < max_length:
            ...
            last_attention_weight = torch.softmax(outputs.cross_attentions[-1], dim=-1)
        
            p_copy = torch.sigmoid(linear_copy(last_hidden_state))
        
            next_token_logits = next_token_logits.unsqueeze(1)
            previous_word_pro = torch.softmax(next_token_logits, dim=-1) * (1 - p_copy)
            
            encoder_word_attention = p_copy * torch.mean(last_attention_weight, dim=1)

            mask = torch.where(encoder_input_ids == 3, encoder_word_attention.new_zeros(encoder_input_ids.shape),
                    encoder_word_attention.new_ones(encoder_input_ids.shape))
            mask = mask.repeat(num_beams, 1)
            
            encoder_word_attention = encoder_word_attention * mask.unsqueeze(1)

            personal_words = encoder_input_ids.unsqueeze(1).repeat(1*num_beams, encoder_word_attention.shape[1], 1)
        
            word_pro = torch.scatter_add(previous_word_pro, 2, personal_words, encoder_word_attention)
            next_token_scores = word_pro.squeeze(1)
